...
public class Utility {
	...
	public static String paging(int totalRecord, int nowPage, int recordPerPage,  String url) {
		...
		if (nowGrp >= 2) { // 내가 보고 있는 그룹이 2그룹이면(6~10)
			str.append(
					"<li><a href='" + url +  "&nowPage=" + _nowPage + "'>이전</A></li>");
		}
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (nowPage == i) {
				...
				str.append("<li><a href='" + url  + "&nowPage=" + i + "'>" + i
						+ "</A></li>");
			}
		}
		...
		if (nowGrp < totalGrp) {
			str.append(
					"<li><A href='" + url  +  "&nowPage=" + _nowPage + "'>다음</A></li>");
		}
		str.append("</ul>");
		str.append("</div>");
		return str.toString();
