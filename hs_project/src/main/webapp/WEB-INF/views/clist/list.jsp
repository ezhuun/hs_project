...
.item-date{
	...
	<div class="container-inner sideBorder boxsing">
		...
		<c:forEach var="dto" items="${list}" varStatus="status">
			...
			<span class="todo-wrap">
				<input type="checkbox" id="${dto.c_num}" class="checkItem" 
				<c:if test="${dto.checked==1}">
				checked
				</c:if>
				/>
				<label for="${dto.c_num}" class="todo">
	...
	$(".checkItem").click(function(){
		...
		$.ajax({
			 ...
			 data:{"c_num":currentId, "checked":checked},
	...
	$('#add-todo').click(function () {
		...
		$(this).before('<span class="editing todo-wrap"><input type="checkbox" class="checkItem" id="' + newId + '"/><label for="' + newId + '" class="todo"><i class="fa fa-check"></i><input type="text" class="input-todo" id="input-todo' + newId + '" /></label></div>');
		...
		$('#input-todo' + newId + '').on('blur enterEvent', function () {
			...
			if (todoTitleLength > 0) {
				...
				$.ajax({
					 ...
					 success:function(data){
						 newId = data;
						 $('.todo-wrap:last-of-type > input').attr("id", data);
						 $('.todo-wrap:last-of-type > label').attr("for", data);
					 },
					 error:function(xhr, status, errorThrown){
					 }
				});
